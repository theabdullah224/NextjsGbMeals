...
function Pref() {
  ...
  const [menuStates, setMenuStates] = useState({
    ...
    preference: { open: false, selected: "" },
    dislike: { open: false, selected: "" },
    mealPlan: { open: false, selected: "" },
    familyMembers: { open: false, selected: "" },
  });

  const handleInputChange = (menu: string, value: string) => {
    setMenuStates({
      ...menuStates,
      [menu]: {
        ...menuStates[menu],
  ...
  const renderMenuItem = (menu: string, title: string, placeholder: string, options: string[]) => (
    <li className="relative ">
      <span className="text-base font-roboto font-bold">{title}</span>
      <button
        onClick={() => isEditable && handleMenuClick(menu)}
        className={`text-sm text-Text2 border-2 rounded-lg border-[#A6AE9D]  px-4 py-3 w-[75vw] sm:w-[30rem] flex justify-between items-center ${!isEditable && "opacity-50 cursor-not-allowed"}`}
        disabled={!isEditable}
      >
      ...
      {menuStates[menu].open && (
        <ul className="absolute z-40 left-0 mt-2 bg-white border border-gray-300 w-full px-2 py-2">
          ...
          {options.map((option) => (
            <li key={option}>
              <span onClick={() => handleOptionSelect(menu, option)} className="block px-4 py-2 hover:bg-gray-300 cursor-pointer">
                {option}
              </span>
            </li>
          ))}
  ...
  const handleOptionSelect = (menu: any, option: any) => {
  ...
  const handleGeneratePDF = () => {
    const payload = {
      ...
      dislikes: menuStates.dislike.selected,
      prefMeal: menuStates.mealPlan.selected,
      totalCalories: menuStates.preference.selected,
      days: "7 days",
      id:UserId,
    };
    try {
      const res = axios.post("/api/generate",payload);
        ...
        <div className="max-w-[40rem]  my-[2rem] sm:my-[5rem] xl:px-14 mx-auto">
          ...
          <div className=" flex  flex-col justify-center items-center xl:justify-start xl:items-start">
            <ul className="w-full gap-4 flex flex-col xl:flex-row items-center xl:flex-wrap">
              ...
              {renderMenuItem("servings", "How many servings are required", `${menuStates.servings.selected || "Select servings"}`, ["1 serving", "2 servings", "3 servings", "4 servings"])}
              {renderMenuItem(
                "allergy",
              ...
              {renderMenuItem("mealPlan", "Choose your preferred meal plan", menuStates.mealPlan.selected || "select preferred meal plan", [
              ...
              {renderMenuItem("preference", "Choose Your Preferred Calories", menuStates.preference.selected || "Enter/select preferred Calories", [
